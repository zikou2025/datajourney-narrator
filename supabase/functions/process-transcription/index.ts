...
async function processTranscriptionWithAI(text: string, preferredProcessor?: string): Promise<LogEntry[]> {
  try {
    ...
    try {
      ...
      if (!preferredProcessor || preferredProcessor === 'claude') {
        try {
          console.log('Attempting to process with Claude...')
          const claudeKey = Deno.env.get('CLAUDE_API_KEY')
          if (claudeKey) {
            const enhancedLogs = await enhanceLogsWithClaude(text, claudeKey)
            if (enhancedLogs && enhancedLogs.length > 0) {
              console.log(`Successfully generated ${enhancedLogs.length} logs with Claude`)
              return enhancedLogs
            }
          }
        } catch (claudeError) {
          console.error('Error enhancing with Claude:', claudeError)
        }
      }
      ...
      console.error('All AI services failed, using rule-based logs:', aiError)
...
// New function to use Claude for log enhancement
async function enhanceLogsWithClaude(text: string, apiKey: string): Promise<LogEntry[]> {
  try {
    console.log('Calling Claude API with structured prompt...')
    
    const prompt = `
    <task>
    Extract structured activity logs from industrial transcription data. You are an expert in analyzing industrial operations transcriptions.
    </task>
    ...
    <output_format>
    Extract all distinct activities mentioned in the transcription and return ONLY a JSON array containing structured log entries. Each entry should have these fields:
    - location: The specific site where the activity takes place
    - activityCategory: Categorize as Installation, Maintenance, Monitoring, Construction, Transportation, Extraction, Processing, or another relevant category
    - activityType: The specific action being performed (be detailed and accurate)
    ...
    Format as a valid JSON array of objects, with no additional text or explanation.
    </output_format>
    `
    ...
    if (data.content && data.content.length > 0) {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          const enhancedLogs = logsFromAI.map((logItem, index) => {
            return {
              ...
              notes: logItem.notes || '',
              referenceId: `REF-CLD-${Date.now().toString().slice(-5)}-${index}`,
        ...
        console.error('Error parsing JSON from Claude:', jsonError)
